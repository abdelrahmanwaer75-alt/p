...
object BackgroundDownloadBridge {
    ...
    @Synchronized
    fun attach(appContext: Context, eventSink: EventChannel.EventSink?) {
    ...
    fun emit(taskId: String, event: String, fields: Map<String, Any?> = emptyMap()) {
        ...
        payload["event"] = if (event.startsWith("download.")) event else "download.$event"
    ...
    fun start(arguments: Map<*, *>): Boolean {
        ...
        val data = Data.Builder()
            ...
            .putString("url", url)
            .putString("filename", arguments["filename"]?.toString() ?: "$taskId.bin")
            .build()
    ...
    fun pause(taskId: String): Boolean {
        context?.getSharedPreferences(BackgroundDownloadWorker.PREFS, Context.MODE_PRIVATE)
            ?.edit()
            ?.putBoolean("paused_$taskId", true)
            ?.apply()
        emit(taskId, "paused")
        return true
    }
    ...
    fun resume(taskId: String): Boolean {
        context?.getSharedPreferences(BackgroundDownloadWorker.PREFS, Context.MODE_PRIVATE)
            ?.edit()
            ?.putBoolean("paused_$taskId", false)
            ?.apply()
        emit(taskId, "started")
        return true
    }
    ...
    fun cancel(taskId: String): Boolean {
        val app = context ?: return false
        WorkManager.getInstance(app).cancelUniqueWork("vidora_download_$taskId")
