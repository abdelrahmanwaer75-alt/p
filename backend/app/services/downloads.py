...
class DownloadService:
    def __init__(self, repository: DownloadRepository, queue: DownloadQueue | None = None) -> None:
    ...
    def create(self, payload: DownloadTaskCreate, owner_id: UUID) -> DownloadTask:
        ...
        task = self._repository.create(payload, owner_id)
    ...
    async def run(self, task_id: UUID, owner_id: UUID) -> DownloadTask:
        ...
        if task.status in {DownloadStatus.COMPLETED, DownloadStatus.CANCELLED}:
            return task
        task = task.model_copy(update={"status": DownloadStatus.QUEUED, "updated_at": datetime.now(timezone.utc)})
