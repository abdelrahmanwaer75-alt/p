...
class PlayerController extends StateNotifier<PlayerState> {
  ...
  Future<void> openFile(
    ...
    if (path.trim().isEmpty) {
      state = state.copyWith(error: 'A local file path is required');
      return;
    }
    try {
      final items =
          playlist ?? (item == null ? const <PlaylistItem>[] : [item]);
      await player.open(Media(path), play: false);
      state = state.copyWith(
        current: item,
        playlist: items,
        index: items.isEmpty ? -1 : index,
        position: Duration.zero,
        duration: Duration.zero,
        clearError: true,
      );
    } catch (error) {
      state = state.copyWith(error: 'Unable to open local media: $error');
    }
  }

  Future<void> playPlaylist(List<PlaylistItem> items, {int index = 0}) async {
    if (items.isEmpty || index < 0 || index >= items.length) {
      state = state.copyWith(error: 'Playlist is empty');
      return;
    }
    final item = items[index];
    if (item.mediaPath == null || item.mediaPath!.isEmpty) {
      state = state.copyWith(
        error: 'The selected playlist item has no local file',
      );
      return;
    }
  ...
  Future<void> forward([Duration by = const Duration(seconds: 10)]) =>
      player.seek(state.position + by);
  Future<void> backward([Duration by = const Duration(seconds: 10)]) =>
      player.seek(state.position - by);
  ...
  Future<void> setSpeed(double speed) async {
    final value = speed.clamp(0.25, 4).toDouble();
  ...
  Future<void> next() async {
    if (state.index + 1 < state.playlist.length) {
      await playPlaylist(state.playlist, index: state.index + 1);
    }
  }
